...
- [ ] 项目资料库（资料上传、自动解析、RAG 处理、分类管理、版本管理）
...
| Milestone | 内容 | 目标 |
|-----------|------|------|
| M1 | 框架搭建 + 用户认证 + 用户管理 + AI 模型配置 + 文档类型管理 | 基础设施就绪，管理员可配置模型和文档类型 |
| M2 | 流程编排配置（5 种节点编排、节点配置、变量系统、校验） | 管理员可创建和配置完整的文档生成流程 |
| M3 | 项目管理 + 文档创建 + 文档生成工作台 + 导出 | 用户可跑通完整流程生成文档 |
| M4+ | 资料库、统计审计、用量管理、企业微信集成等 | 补齐辅助功能，准备内部推广 |
