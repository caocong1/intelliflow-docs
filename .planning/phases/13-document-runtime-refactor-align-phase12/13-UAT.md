...
result: issue
reported: "进行中的文档只显示'进行中'状态标签，没有进度条，也没有当前节点信息。后端未填充 progressStep/totalSteps/currentNodeLabel 字段。"
severity: minor
...
result: issue
reported: "脱敏节点显示'暂无输入文本，请等待上游节点完成'——上游InputTransform节点已完成但输出数据未传递到脱敏节点。后端advanceNode未正确传递上游输出到下游节点inputData。同时draft端点返回404。自动检测因无输入文本未触发。"
severity: blocker

### 6. 多源脱敏输入（多模型工作流）
expected: 当脱敏节点有多个上游输入源时，显示标签页切换不同输入源，每个输入源独立脱敏处理。
result: skipped
reason: 脱敏节点无法接收上游数据（Test 5 blocker），无法测试多源输入功能

### 7. 模型调用节点执行
expected: 进入模型调用节点，显示中文状态标签，点击执行后通过 SSE 实时流式输出。支持 Markdown/源码 视图切换。未配置模型时显示"未配置模型"提示。
result: skipped
reason: 无法跳过脱敏节点（skip API 返回404），无法到达模型调用节点
...
result: skipped
reason: 无法到达有编辑功能的节点（被Test 5 blocker阻断）
...
- truth: "进行中的文档在文档列表显示进度条和当前节点信息"
  ...
  missing:
    - "在listDocuments select中添加3个子查询：progressStep（completed/skipped count）、totalSteps（total count）、currentNodeLabel（in_progress node label）"
    - "更新DocumentListItem类型添加3个可选字段"
  debug_session: ""
...
- truth: "脱敏节点接收上游InputTransform输出并自动触发检测"
  ...
  artifacts:
    ...
    - path: "packages/frontend/src/pages/workspace/DocumentWorkspace.tsx"
      issue: "debouncedDraftSave body未包装为{data: ...}（line 75），handleInlineEditorSave同样（line 177）"
  ...
  missing:
    - "在input-transform.service.ts的outputData中添加text: combinedText字段"
    - "在DocumentWorkspace.tsx:75将body改为JSON.stringify({ data })"
    - "在DocumentWorkspace.tsx:177将body改为JSON.stringify({ data: { text: content } })"
  debug_session: ""
