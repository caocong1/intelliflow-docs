          ...
          >
            {(field, index) => (
              <div class="bg-slate-50 rounded-lg border border-slate-200 p-3">
                {/* Top row: move buttons + label input + type select + delete */}
                <div class="flex items-center gap-2">
                  ...
                  <select
                    value={field.type}
                    onChange={(e) =>
                      updateField(field.id, { type: e.currentTarget.value as FormFieldDef["type"] })
                    }
                    class="text-xs px-1.5 py-1.5 border border-gray-300 rounded-md bg-white text-slate-700 focus:outline-none focus:ring-1 focus:ring-indigo-400 cursor-pointer flex-shrink-0"
                  >
