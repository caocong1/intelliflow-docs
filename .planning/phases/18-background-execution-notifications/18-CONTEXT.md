...
**关键认知：** 后台执行是后端的透明运行模式，不是一个面向用户的独立功能。前端工作区体验和之前几乎完全一样，只是数据源从前端状态变成后端状态。不需要独立的任务列表页面、侧边栏任务入口、后台触发按钮。通知抽屉增加「任务」标签页提供跨项目任务全局视图（BGND-03 / SC-2 要求）。
...
- ~~无并发上限~~ → **已更新**：每用户最多 3 个并发后台任务（ROADMAP SC-6 要求，作为 v2 配额系统前的临时保护措施）。超过上限返回 429 错误
