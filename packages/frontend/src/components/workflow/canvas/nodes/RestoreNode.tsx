    ...
    <div
      class={`min-w-[180px] rounded-lg overflow-hidden cursor-pointer transition-all duration-200 ${
        hasError()
          ...
          : "bg-green-50/50 shadow-sm hover:shadow-md"
      }`}
      style={{ "border-left": hasError() ? "4px solid #ef4444" : "4px solid #22c55e" }}
    >
      <div class="px-3 py-2.5">
        <div class="flex items-center gap-2">
          <div class={`w-6 h-6 rounded flex items-center justify-center flex-shrink-0 ${hasError() ? "bg-red-100" : "bg-green-100"}`}>
            <svg class={`w-3.5 h-3.5 ${hasError() ? "text-red-600" : "text-green-600"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
              <title>信息恢复</title>
              <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 10.5V6.75a4.5 4.5 0 119 0v3.75M3.75 21.75h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H3.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
            </svg>
          </div>
          <span class={`text-xs font-semibold flex-1 truncate ${hasError() ? "text-red-700" : "text-green-700"}`}>
            {props.data.label}
          </span>
          <span
