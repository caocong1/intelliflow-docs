...
export function evaluateExecutionRule(
  ...
  for (const cond of rule.conditions) {
    const resolvedValue = resolveRef(
      {
        nodeId: cond.sourceRef.nodeId,
        outputId: cond.sourceRef.outputId,
        fieldPath: cond.sourceRef.fieldPath,
      },
      nodeExecs,
    );
  ...
  const triggered =
    rule.logic === "and" ? results.every((r) => r.passed) : results.some((r) => r.passed);
  ...
  const metConditions = results
    ...
    .map((r) => {
      ...
      switch (r.cond.operator) {
        case "exists":
          return `${r.cond.sourceRef.variableName} 存在`;
        case "not_exists":
          return `${r.cond.sourceRef.variableName} 不存在`;
        case "equals":
          return `${r.cond.sourceRef.variableName} == "${r.cond.value}" (当前值: ${val})`;
        case "not_equals":
          return `${r.cond.sourceRef.variableName} != "${r.cond.value}" (当前值: ${val})`;
        case "contains":
          return `${r.cond.sourceRef.variableName} 包含 "${r.cond.value}"`;
        default:
          return `${r.cond.sourceRef.variableName} ${r.cond.operator} ${r.cond.value ?? ""}`;
      }
    });
