      ...
      if (client.composeSlide && coherence.slideFixes.length > 0) {
        ...
        for (const fix of uniqueFixes) {
          ...
          try {
            ...
            const validated = validateDeckSlide({ ...original, ...(rewritten as Partial<typeof original>) });
            if (validated.ok) {
        ...
        deckPlan = { ...deckPlan, slides: deckPlan.slides.map((slide) => byId.get(slide.id) ?? slide) };
      }
