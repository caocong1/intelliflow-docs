# RAW: daymade/claude-code-skills — ppt-creator

> Captured: 2026-05-20 via curl.
> Repo path: github.com/daymade/claude-code-skills/tree/main/daymade-docs/ppt-creator
> Marketplace listing: https://explainx.ai/skills/daymade/claude-code-skills/ppt-creator
> Part of "daymade/claude-code-skills" — 52+ production-ready Claude Code skills marketplace.
> Install: `claude plugin install ppt-creator@daymade-skills`

---

## Repo overview

Parent repository `daymade/claude-code-skills` is a professional marketplace of Claude Code skills.

Top-level dirs include:
- `daymade-skill/` — skill creation framework
- `daymade-docs/` — documentation-focused skills suite (where ppt-creator lives)
- `daymade-claude-code/` — Claude Code operations suite
- `demos/` — interactive demonstration GIFs

`ppt-creator` is a directory under `daymade-docs/`.

---

## SKILL.md (verbatim — fetched from `daymade-docs/ppt-creator/SKILL.md`)
...
- Cover, TOC, Problem Statement, Opportunity/Goal, Solution Overview, Evidence 1-3, Risk & Mitigation, Case Study/Comparison, Roadmap/Timeline, Conclusion & Actions, Backup Slides
...
3. Slide Assertions (headings are assertion sentences)
4. Evidence Quality
5. Chart Fit
6. Visual & Accessibility
7. Coherence & Transitions
8. Speakability (45-60 sec/slide)
...
- Uses standard Anthropic Skills YAML frontmatter (`name:`, `description:`).
- `references/` directory pattern (matches Anthropic's official skill template).
- `scripts/chartkit.py` is the only executable; everything else is markdown reference.
- Dual-path PPTX generation: Marp CLI + Anthropic's `document-skills:pptx` (the built-in PPTX skill) — produces TWO PPTX files for user comparison.
- Pyramid Principle + assertion-evidence storytelling is the core IP.

## Related: lewislulu/html-ppt-skill (briefly)

Mentioned as reference. Pure HTML/CSS/JS — no PPTX.
- 36 CSS themes, 15 full-deck templates, 31 layouts, 47 animations (27 CSS + 20 canvas FX).
- Presenter mode (S key) with 4 magnetic draggable cards: CURRENT slide / NEXT slide / SCRIPT (逐字稿) / TIMER.
- Pixel-perfect previews via `<iframe>` with `?preview=N` query param (same CSS/theme/fonts as audience view).
- Cross-window sync via `postMessage` + `BroadcastChannel`.
- 演讲者模式 + 逐字稿 (speaker script) authoring guide: 150-300 words per slide, 2-3 minutes/page rhythm, conversational tone.
- File structure: `SKILL.md` (dispatcher) + `assets/` + `templates/` + `scripts/new-deck.sh` + `scripts/render.sh`.
- Uses standard frontmatter (`name: html-ppt`, with rich trigger keywords list).
